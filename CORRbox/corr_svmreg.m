function [Yhat, pred] = corr_svmreg(X, Y, varargin)

% CORR_SVMREG performs a support vector machine (SVM) regression for
% correlation-valued covariates and scalar responses. Given the number of
% folds and the choice of kernel, it performs cross-validation and
% hyperparameter optimization for automated learning.
%
%   * USAGE
%       [Yhat, pred] = corr_svmreg(X, Y)
%       [Yhat, pred] = corr_svmreg(X, Y, Name, Value)
%
%   * INPUT
...
%       'kernel'   - name of the kernel function; should be one of
%                    "linear", "gaussian", "rbf", or "polynomial" with
%                    order 2 (default: "gaussian").
...
many_kern = ["linear","gaussian","rbf","polynomial"];
...
valid_kern = @(x) (isstring(x) && any(contains(many_kern, lower(x))));
...
addParameter(myparser, "kernel", base_kern, valid_kern);
...
if (length(par_Y)~=size(par_X.data,3))
    error("* corr_svmreg : length of 'Y' must be equal to the data size of 'X'.");
end
par_geom = lower(parsed.geometry);
par_fold = max(2, round(parsed.Kfold));
par_kern = lower(parsed.kernel);
...
%  train
if (strcmpi(par_kern, "polynomial"))
    learned_model = fitrsvm(X_local, par_Y, ...
...
pred = @(C3dnew) geomobj.pred_kernreg(C3dnew, Cref, learned_model, "corr_svmreg");
